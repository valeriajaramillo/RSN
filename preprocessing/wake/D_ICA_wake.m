...
addpath(genpath('/users/psychology01/Valeria/Scripts'));

Folderpath = '/users/m17462/psychology01/parallel_scratch/projects/RSN/RSN_024'; % wihtout / at the end
Folderpath_dir = dir([Folderpath,filesep,'*czref_goodwake.set']);
...
for f = 1:size(Folderpath_dir,1)
    

    batch(cluster,@ICA,0,{Folderpath,Folderpath_dir,f},'AutoAttachFiles', false, ...
...
function ICA(Folderpath,Folderpath_dir,f)
    
    filename = Folderpath_dir(f).name;
    ...
    ica_dir = [Folderpath,'/ICA_', dataName];
    mkdir(ica_dir)  
    ...
    runamica15(eeg, 'num_chans', eeg.nbchan,...
        'outdir', [Folderpath,'/ICA_', dataName],...
        'pcakeep', dataRank, 'num_models', 1,...
        'do_reject', 1, 'numrej', 15, 'rejsig', 3, 'rejint', 1, 'max_threads', 1);
        
    eeg.etc.amica  = loadmodout15([Folderpath,'/ICA_', dataName]);
  ...
  templateChannelFilePath = '/users/psychology01/software/eeglab/plugins/dipfit4.3/standard_BEM/elec/standard_1005.elc';
    ...
    eeg = pop_saveset(eeg, 'filename', [dataName,'_ICA'], 'filepath', [Folderpath,'/ICA_', dataName]);
...
%     mkdir([Folderpath,dataName,'/IClabel'])
    ...
    % Save the dataset
    eeg = pop_saveset(eeg, 'filename', [dataName,'_ICs_removed'], 'filepath', [Folderpath,'/ICA_', dataName]);
