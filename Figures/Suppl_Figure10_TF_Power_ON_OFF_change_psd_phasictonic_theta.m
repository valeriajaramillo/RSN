...
fig = figure('Renderer','painters','units','normalized','outerposition',[0 0 1 1])
    ...
    psd_on_ch_allcon = squeeze(nanmean(nanmean(mpsd_con_ch_stage(incl_sub,ch,5:8,:,all_ep),2),3));
    m_psd_off_ch_allcon = nanmean(squeeze(nanmean(nanmean(mpsd_con_ch_stage(incl_sub,ch,5:8,:,off_block),2),3)),3);
...
%% Theta Peak vs Trough

fig = figure('Renderer','painters','units','normalized','outerposition',[0 0 1 1])

% subplot(2,4,5)

psd_on_ch_peak = squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,5,:,all_ep),2));
m_psd_off_ch_peak = nanmean(squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,5,:,off_block),2)),3);
...
psd_on_ch_trough = squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,7,:,all_ep),2));
m_psd_off_ch_trough = nanmean(squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,7,:,off_block),2)),3);
...
psd_on_ch_falling = squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,6,:,all_ep),2));
m_psd_off_ch_falling = nanmean(squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,6,:,off_block),2)),3);
...
psd_on_ch_rising = squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,8,:,all_ep),2));
m_psd_off_ch_rising = nanmean(squeeze(nanmean(mpsd_con_ch_stage(incl_sub,ch,8,:,off_block),2)),3);
